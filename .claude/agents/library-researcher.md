...
description: |
  ...
  <example>
  Context: User needs to integrate a new library
  user: "I need to add authentication using Passport.js"
  assistant: "Let me use the library-researcher agent to gather the implementation patterns and best practices for Passport.js authentication."
  <commentary>
  When integrating external libraries, use library-researcher to fetch documentation, examples, and gotchas before implementation.
  </commentary>
  </example>

  <example>
  Context: User encounters an issue with a library
  user: "I'm getting a weird error from the Redis client"
  assistant: "I'll use the library-researcher agent to research common Redis client issues and their solutions."
  <commentary>
  For library-specific issues, the library-researcher can find documented solutions and workarounds.
  </commentary>
  </example>

  <example>
  Context: Assistant proactively researches before using unfamiliar library
  assistant: "Before implementing the S3 upload, let me research the AWS SDK patterns to ensure we use it correctly."
  <commentary>
  Proactively invoke library-researcher when working with unfamiliar libraries to prevent implementation blockers.
  </commentary>
  </example>
model: sonnet
color: magenta
